...
Official Windows releases use the AstraClaw Team Certum cloud code-signing certificate through SimplySign Desktop, with the expected publisher `RushRush Network Technology Ltd`. Private signing material is never stored in Git or GitHub Actions Secrets.
...
- expose exactly one unexpired code-signing certificate for `RushRush Network Technology Ltd` in `Cert:\\CurrentUser\\My`;
